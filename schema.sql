...
CREATE TABLE friends (
  ...
  u2 NUMBER(15),
  PRIMARY KEY (u1, u2),
  FOREIGN KEY (u1) REFERENCES users(uid) ON DELETE CASCADE,
  FOREIGN KEY (u2) REFERENCES users(uid) ON DELETE CASCADE
);
...
CREATE TABLE  mc_msg_recipients (
  ...
  FOREIGN KEY (recipient) REFERENCES users(uid) ON DELETE CASCADE,
...
CREATE TABLE chat_groups (
  ...
  PRIMARY KEY (cgid),
  FOREIGN KEY (owner) REFERENCES users(uid) ON DELETE SET NULL
);
